...
void BuildProblem(const BALProblem* bal_problem,
                  g2o::SparseOptimizer* optimizer, const BundleParams& params)
{
  ...
  for (int j = 0; j < num_points; ++j)
  {
    ...
    pPoint->setId(j + num_cameras);  // each vertex should have an unique id, no
                                     // matter it is a camera vertex, or a point
                                     // vertex
  ...
  const double* observations =
      bal_problem->observations();  // pointer for the first observation..
  ...
  for (int i = 0; i < num_observations; ++i)
  {
    ...
    const int camera_id =
        bal_problem->camera_index()[i];  // get id for the camera;
    const int point_id =
        bal_problem->point_index()[i] + num_cameras;  // get id for the point
...
void WriteToBALProblem(BALProblem* bal_problem, g2o::SparseOptimizer* optimizer)
{
  ...
  for (int i = 0; i < num_cameras; ++i)
  {
    VertexCameraBAL* pCamera =
        dynamic_cast<VertexCameraBAL*>(optimizer->vertex(i));
    Eigen::VectorXd NewCameraVec = pCamera->estimate();
    memcpy(raw_cameras + i * camera_block_size, NewCameraVec.data(),
           sizeof(double) * camera_block_size);
  }

  double* raw_points = bal_problem->mutable_points();
  for (int j = 0; j < num_points; ++j)
  {
    VertexPointBAL* pPoint =
        dynamic_cast<VertexPointBAL*>(optimizer->vertex(j + num_cameras));
    Eigen::Vector3d NewPointVec = pPoint->estimate();
    memcpy(raw_points + j * point_block_size, NewPointVec.data(),
           sizeof(double) * point_block_size);
  }
}

// this function is  unused yet..
void SetMinimizerOptions(std::shared_ptr<BalBlockSolver>& solver_ptr,
                         ...
                         g2o::SparseOptimizer* optimizer)
{
  ...
  {
    std::cout << "Please check your trust_region_strategy parameter again.."
              << std::endl;
    exit(EXIT_FAILURE);
  }
...
// this function is  unused yet..
void SetLinearSolver(std::shared_ptr<BalBlockSolver>& solver_ptr,
                     const BundleParams& params)
{
  ...
  else if (params.linear_solver == "sparse_schur")
  {
    ...
    dynamic_cast<g2o::LinearSolverCholmod<BalBlockSolver::PoseMatrixType>*>(
        linearSolver)
        ->setBlockOrdering(true);  // AMD ordering , only needed for sparse
                                   // cholesky solver
  }
